  ...
  m.draw = function() {		
    ...
    m.rotate(m.rotation);
      ...
      for(let i = 0; i < m.angle1.length; i++) {
        m.stroke(255-i*(0.05*i));
        m.line( m.cos(m.angle1[i])*m.r,  m.sin(m.angle1[i])*m.r,  m.cos(m.angle2[i])*m.r,  m.sin(m.angle2[i])*m.r);
      }
      m.pop();
    ...
    for(let i = 0; i < m.specXDisplay.length; i++) {
      m.specColor =  m.map(i, 0, m.specXDisplay.length, 1, 0);
    ...
    //Atualizar
    m.angle1.push( m.random( m.TWO_PI)); m.angle1.shift();
    m.angle2.push( m.random( m.TWO_PI)); m.angle2.shift();
    
    for(let i = 0; i < 6; i++) {
      m.index =  m.floor( m.random(m.specX.length));
      m.specXDisplay.push(m.specX[m.index]); m.specXDisplay.shift();
      m.specYDisplay.push(m.specY[m.index]); m.specYDisplay.shift();
    }
    
    m.rotation += ( m.TWO_PI/360)/15;
  };
